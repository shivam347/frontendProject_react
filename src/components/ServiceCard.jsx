...
const ServiceCard = ({service, index}) => {
    ...
    const divRef = useRef(null)  // gives direct access to dom element

    const handleMouseMove = (e)=>{
        const bounds = divRef.current.getBoundingClientRect();  // Getting the box size using getBoundingclientRect
        setPosition({x : e.clientX - bounds.left, y : e.clientY - bounds.top} ) // calculating mouse position inside the card
    }
  ...
  return (
    ...
    dark:shadow-white/10' onMouseEnter={()=>setVisible(true)} onMouseLeave={()=> setVisible(false)} ref={divRef} onMouseMove={handleMouseMove} >
        ...
        via-indigo-500 to-purple-500 w-[300px] absolute z-0
